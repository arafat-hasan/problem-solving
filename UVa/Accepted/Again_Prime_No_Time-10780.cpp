 ...
 *
 * LINK: 10780 - Again Prime? No Time.
 *
 * Description:
 *
 *
 ...
 * --------------------------------------------------------------
 *    28 Jan 2017    New             1.0         Not Completed
 *
 *
 */
...
int main() {
    ...
    int n, m, i, t, numOFfact, count, j, c = 0;
    cin >> t;
    while(c<t) {
        cin >> m >> n;
        count = 0;
        for(i = n; i>1; i--) {
            numOFfact = primefactor(i);
			//cout << "#"<<numOFfact<<endl;
            for(j = 0; j< numOFfact; j++)
                if(fact[j][0] == m)
                    count+=fact[j][1];

        }
        
        //count = primefactor(10);
        cout << "Case " << ++c << ":" <<endl
				<< count+1 <<endl;
